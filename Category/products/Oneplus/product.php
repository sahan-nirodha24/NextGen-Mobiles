...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES["product_image"]["error"] == UPLOAD_ERR_OK) {
        if (move_uploaded_file($_FILES["product_image"]["tmp_name"], $targetFilePath)) {
            ...
            $sql = "INSERT INTO oneplus (product_name, product_price, product_image, product_quantity, discount, rating) 
                    VALUES ('$productName', '$productPrice', '$targetFilePath', '$productQuantity', '$discount', '$rating')";

            if ($conn->query($sql) === TRUE) {
                echo "<script>
                        alert('Product added successfully!');
                        window.location.href = 'oneplus.php';
                     </script>";
            } else {
